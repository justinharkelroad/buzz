...
pub async fn list_relay_agents(state: State<'_, AppState>) -> Result<Vec<RelayAgentInfo>, String> {
    // kind:10100 is agent-authored directory/add-policy metadata. The runtime
    // invocation policy is published separately by the owner as kind:30177.
    // Fetch both, then resolve the named agents' kind:0 profiles so the pure
    // merge can cryptographically bind each 30177 author to its NIP-OA agent.
    let discovery_events = query_relay(
        &state,
        &[
            serde_json::json!({ "kinds": [10100] }),
            serde_json::json!({ "kinds": [30177] }),
        ],
    )
    .await?;

    let agent_pubkeys =
        nostr_convert::agent_identity_pubkeys_from_discovery_events(&discovery_events);
    let identity_profiles = if agent_pubkeys.is_empty() {
        Vec::new()
    } else {
        let profile_limit = agent_pubkeys.len();
        query_relay(
            &state,
            &[serde_json::json!({
                "kinds": [0],
    ...
    let (agent_profiles, managed_agents): (Vec<_>, Vec<_>) = discovery_events
        .into_iter()
        .partition(|event| event.kind.as_u16() == 10100);
    Ok(nostr_convert::relay_agents_from_discovery_events(
        &agent_profiles,
        &managed_agents,
        &identity_profiles,
    ))
