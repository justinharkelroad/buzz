...
impl FramePublisher {
    ...
    pub(crate) fn offer(&mut self, frame: Frame) -> Result<Option<Publication>, OfferError> {
        ...
        Ok(Self::publish(subscription, frame))
    }
    ...
    fn flush(subscription: &mut Subscription) -> Option<Publication> {
        ...
        Self::publish(subscription, frame)
    }

    fn publish(subscription: &mut Subscription, frame: Frame) -> Option<Publication> {
        debug_assert!(subscription.in_flight.is_none());
        let sequence = subscription.next_sequence;
        // Sequence IDs must never wrap or be reused. Exhaustion therefore
        // closes publication for this attachment without panicking.
        subscription.next_sequence = sequence.checked_add(1)?;
...
mod tests {
    ...
    #[test]
    fn sequence_exhaustion_stops_publication_without_panicking_or_reusing_ids() {
        let v0 = viewport(0, 80, 24);
        let mut publisher = FramePublisher::new(v0);
        let id = SubscriptionId::new();
        publisher.attach(id, frame(v0, 0, true)).unwrap();
        ...
        let subscription = publisher.subscription.as_mut().unwrap();
        subscription.next_sequence = u64::MAX;

        assert_eq!(publisher.offer(frame(v0, 1, true)).unwrap(), None);
        assert_eq!(publisher.offer(frame(v0, 2, true)).unwrap(), None);

        let subscription = publisher.subscription.as_ref().unwrap();
        assert_eq!(subscription.next_sequence, u64::MAX);
        assert_eq!(subscription.in_flight, None);
    }
}
