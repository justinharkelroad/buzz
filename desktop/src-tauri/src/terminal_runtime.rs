...
/// Maximum retained grid area, including scrollback, for one terminal.
const MAX_TERMINAL_GRID_CELLS: usize = 1024 * 1024;
const READER_JOIN_GRACE: Duration = Duration::from_millis(250);
const READER_JOIN_POLL: Duration = Duration::from_millis(5);

type Result<T> = std::result::Result<T, String>;

fn require_main_window_label(label: &str) -> Result<()> {
    if label == "main" {
        Ok(())
    } else {
        Err("terminal commands are restricted to the main window".to_string())
    }
}
    ...
    fn join(mut self: Box<Self>) {
        if let Some(handle) = self.handle.take() {
            ...
            if handle.is_finished() {
                ...
                // A detached process may deliberately retain the PTY slave.
                // Never turn that external lifetime into an unbounded UI hang;
                // dropping the handle detaches this already-stopping reader.
                eprintln!(
                    "buzz-terminal: PTY reader did not stop within {:?}; detaching it",
                    READER_JOIN_GRACE
                );
            }
        }
...
fn size(columns: u16, rows: u16) -> Result<Size> {
    ...
    if columns > MAX_TERMINAL_COLUMNS || rows > MAX_TERMINAL_ROWS {
        return Err(format!(
            "terminal dimensions exceed the {MAX_TERMINAL_COLUMNS}x{MAX_TERMINAL_ROWS} limit"
        ));
    }

    let columns = usize::from(columns);
    let screen_lines = usize::from(rows);
    let screen_cells = columns
        .checked_mul(screen_lines)
        .ok_or_else(|| "terminal dimensions overflow the cell budget".to_string())?;
    if screen_cells > MAX_TERMINAL_GRID_CELLS {
        return Err("terminal viewport exceeds the cell budget".to_string());
    }

    // Keep the ordinary 10k-line history where it fits, but reduce it for a
    // very wide viewport so `columns * (rows + scrollback)` is always bounded.
    let retained_lines = MAX_TERMINAL_GRID_CELLS / columns;
    let scrollback = Size::default()
        .scrollback
        .min(retained_lines.saturating_sub(screen_lines));
    Ok(Size {
        columns,
        screen_lines,
        scrollback,
