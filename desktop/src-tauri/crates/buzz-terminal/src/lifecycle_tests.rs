...
use rustix::process::{test_kill_process, waitid, Pid, WaitId, WaitIdOptions};
...
fn pid_alive(pid: i32) -> bool {
    Pid::from_raw(pid).is_some_and(|pid| test_kill_process(pid).is_ok())
}

/// Observe child exit while deliberately leaving the zombie waitable.
fn pid_exited_without_reaping(pid: i32) -> bool {
...
/// A leader may exit before the tab is closed while a background descendant
/// remains in its process group and keeps the PTY slave open. Cleanup must use
/// the unreaped leader as a safe PGID anchor, sweep the descendant, and let the
/// draining reader reach EOF instead of hanging in `join`.
#[test]
fn exited_leader_still_sweeps_descendants_and_releases_the_reader() {
    let dir = tempdir("buzz-terminal-exited-leader");
    let pidfile = dir.join("grandchild.pid");
    let armed = dir.join("armed");
    ...
    cmd.arg(format!(
        "sh -c 'trap \"\" HUP TERM; (sleep {WATCHDOG}; kill -9 $$) & \
         : > {armed}; while :; do :; done' & \
         echo $! > {pidfile}; \
         while [ ! -f {armed} ]; do sleep 0.01; done; exit 0",
        armed = armed.display(),
        pidfile = pidfile.display()
    ));
    let mut child = pair.slave.spawn_command(cmd).expect("spawn");
    let leader = child.process_id().expect("leader pid") as i32;

    let order = std::sync::Arc::new(std::sync::Mutex::new(Vec::new()));
    let stop_at: StopClock = std::sync::Arc::new(std::sync::Mutex::new(None));
    let reader = RecordingReader::spawn(&pair, leader, order.clone(), stop_at);
    drop(pair.slave);

    let descendant: i32 = read_when_written(&pidfile)
        .expect("grandchild never reported its pid")
        .parse()
        .expect("pid is a number");
    assert!(
        poll_until(|| armed.exists()),
        "grandchild never armed traps"
    );
    assert!(
        poll_until(|| pid_exited_without_reaping(leader)),
        "session leader did not exit before shutdown"
    );
    assert!(pid_alive(descendant), "grandchild exited before cleanup");

    let started = Instant::now();
    let outcome = shutdown_draining(&mut child, Box::new(reader)).expect("shutdown");
    let elapsed = started.elapsed();

    assert_eq!(outcome, Shutdown::AlreadyExited);
    assert!(
        poll_until(|| !pid_alive(descendant)),
        "descendant survived its exited session leader"
    );
    assert!(
        elapsed < BOUND,
        "reader join hung after exited-leader cleanup: {elapsed:?}"
    );
    assert_eq!(*order.lock().unwrap(), ["begin_closing", "stop", "join"]);
}
