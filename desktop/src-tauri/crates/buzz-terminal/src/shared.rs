...
/// The extra bucket at index 64 represents the remainder of the full `u64`
/// range, so even a pathological multi-hour stall is recorded rather than
/// indexing past the array and panicking on the recovery path.
...
mod tests {
    ...
    #[test]
    fn acquire_meter_covers_the_full_u64_latency_range() {
        let meter = AcquireMeter::default();
        meter.record(u64::MAX);

        let snapshot = meter.snapshot();
        assert_eq!(snapshot.acquisitions, 1);
        assert_eq!(snapshot.max_micros, u64::MAX);
        assert_eq!(snapshot.percentile_micros(1.0), u64::MAX);
    }
