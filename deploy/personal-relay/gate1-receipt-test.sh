...
jq -n '[
  {type: "pull_request", parameters: {required_approving_review_count: 1, dismiss_stale_reviews_on_push: true, require_last_push_approval: true, required_review_thread_resolution: true}, ruleset_id: 777, ruleset_source_type: "Repository", ruleset_source: "justinharkelroad/buzz"},
...
jq -n '{
  ...
  can_admins_bypass_exposed: true,
  ...
  protection_rules: [{
    ...
    reviewers: [{
      type: "User",
      reviewer: {login: "release-reviewer", id: 5151, node_id: "MDQ6VXNlcjUxNTE="}
    }]
  }],
...
jq -n '[{
  state: "approved",
  environments: [{id: 515, node_id: "ENV_release_515", name: "personal-relay-release"}],
  user: {login: "release-reviewer", id: 5151, node_id: "MDQ6VXNlcjUxNTE="}
}]' > "$evidence/personal-relay-release-approval-history.json"
jq -n --arg source_sha "$source_sha" '{
  ...
  actor: {login: "release-actor", id: 1111, node_id: "MDQ6VXNlcjExMTE="},
  triggering_actor: {login: "release-trigger", id: 2222, node_id: "MDQ6VXNlcjIyMjI="}
}' > "$evidence/personal-relay-release-run-identity.json"
jq -n '{
  type: "User",
  reviewer: {login: "release-reviewer", id: 5151, node_id: "MDQ6VXNlcjUxNTE="}
}' > "$evidence/personal-relay-release-reviewer.json"
...
release_approval_history_sha=$(sha256_file "$evidence/personal-relay-release-approval-history.json")
release_run_identity_sha=$(sha256_file "$evidence/personal-relay-release-run-identity.json")
release_reviewer_sha=$(sha256_file "$evidence/personal-relay-release-reviewer.json")
jq -n \
  --arg source_sha "$source_sha" \
  ...
  --arg approval_history_sha "$release_approval_history_sha" \
  --arg run_identity_sha "$release_run_identity_sha" \
  --arg reviewer_sha "$release_reviewer_sha" '
  {
    schema: "personal-relay-release-authorization/v2",
    ...
    approval_history_sha256: $approval_history_sha,
    run_identity_sha256: $run_identity_sha,
    reviewer_sha256: $reviewer_sha,
    admin_bypass_api_state: "disabled",
    admin_bypass_settings_receipt_sha256: "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
  }
...
jq -n \
  ...
  --arg release_approval_history_sha "$release_approval_history_sha" \
  --arg release_run_identity_sha "$release_run_identity_sha" \
  --arg release_reviewer_sha "$release_reviewer_sha" \
  ...
  --slurpfile databases "$evidence/personal-relay-trivy-databases.json" '
  {
    ...
    deployment_blockers: ["separate protected Gate 1 review", "recorded dispositions"],
    ...
    release_approval: {
      ...
      approval_history_sha256: $release_approval_history_sha,
      run_identity_sha256: $release_run_identity_sha,
      reviewer_sha256: $release_reviewer_sha,
      admin_bypass_api_state: "disabled",
      admin_bypass_settings_receipt_sha256: "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb",
      authorization_sha256: $release_authorization_sha,
      evidence_artifact: {
        id: 909,
        name: ("personal-relay-release-approval-" + $source_sha + "-101-1"),
...
jq -n '{
  ...
  protection_rules: [{
    type: "required_reviewers",
    prevent_self_review: true,
    reviewers: [{type: "User", reviewer: {login: "gate-reviewer", id: 4242, node_id: "MDQ6VXNlcjQyNDI="}}]
  }]
...
jq -n '[{state: "approved", user: {login: "gate-reviewer", id: 4242, node_id: "MDQ6VXNlcjQyNDI="}, environments: [{name: "personal-relay-gate1"}]}]' > "$tmp_dir/environment-approvals.json"
jq -n --arg sha "$gate1_sha" '{name: "main", commit: {sha: $sha}, protected: true}' > "$tmp_dir/main-branch.json"
jq -n '[
  {type: "pull_request", parameters: {required_approving_review_count: 1, dismiss_stale_reviews_on_push: true, require_last_push_approval: true, required_review_thread_resolution: true}, ruleset_id: 777, ruleset_source_type: "Repository", ruleset_source: "justinharkelroad/buzz"},
  {type: "deletion", parameters: {}, ruleset_id: 777, ruleset_source_type: "Repository", ruleset_source: "justinharkelroad/buzz"},
  {type: "non_fast_forward", parameters: {}, ruleset_id: 777, ruleset_source_type: "Repository", ruleset_source: "justinharkelroad/buzz"},
...
jq -n --arg gate1_sha "$gate1_sha" '{id: 303, run_attempt: 1, event: "workflow_dispatch", head_sha: $gate1_sha, head_branch: "main", path: ".github/workflows/personal-relay-gate1.yml@main", repository: {full_name: "justinharkelroad/buzz"}, actor: {login: "release-operator", id: 111, node_id: "MDQ6VXNlcjExMQ=="}, triggering_actor: {login: "release-operator", id: 111, node_id: "MDQ6VXNlcjExMQ=="}}' > "$tmp_dir/gate1-run-metadata.json"
...
jq -n \
  ...
  schema: "personal-relay-gate1-source-result/v2",
...
jq '
  .approved_by = {login: "gate-reviewer", id: 4242, node_id: "MDQ6VXNlcjQyNDI="}
  ...
  | .dispositions |= map(
      ...
      | .reviewed_by = {login: "gate-reviewer", id: 4242, node_id: "MDQ6VXNlcjQyNDI="}
...
awk '
  /"approved_by": \{/ && !inserted {
    print
    print "    \"login\": \"ambiguous-reviewer\","
    inserted = 1
    next
  }
  { print }
...
bash "$gate1" validate \
  ...
  --environment-approvals "$tmp_dir/environment-approvals.json" \
...
jq -e --arg source_result_sha256 "$(sha256_file "$source_proof/source-test-result.json")" '
  ...
  and .approval.approved_by == {login: "gate-reviewer", id: 4242, node_id: "MDQ6VXNlcjQyNDI="}
  and .gate1_workflow.configured_reviewers == [{login: "gate-reviewer", id: 4242, node_id: "MDQ6VXNlcjQyNDI="}]
  and .gate1_workflow.approved_reviewers == [{login: "gate-reviewer", id: 4242, node_id: "MDQ6VXNlcjQyNDI="}]
  ...
  and .release_evidence.release_approval.image_name == "ghcr.io/justinharkelroad/buzz-relay-personal"
  and .release_evidence.release_approval.candidate_tag == "sha-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
  and (.release_evidence.release_approval.environment_sha256 | test("^[0-9a-f]{64}$"))
  and (.release_evidence.release_approval.branch_policies_sha256 | test("^[0-9a-f]{64}$"))
  and (.release_evidence.release_approval.approval_history_sha256 | test("^[0-9a-f]{64}$"))
  and (.release_evidence.release_approval.run_identity_sha256 | test("^[0-9a-f]{64}$"))
  and (.release_evidence.release_approval.reviewer_sha256 | test("^[0-9a-f]{64}$"))
  and (.release_evidence.release_approval.authorization_sha256 | test("^[0-9a-f]{64}$"))
  and .release_evidence.release_approval.admin_bypass_api_state == "disabled"
  and .release_evidence.release_approval.admin_bypass_settings_receipt_sha256 == "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
  and .release_evidence.release_approval.evidence_artifact == {
    id: 909,
    name: "personal-relay-release-approval-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa-101-1",
    digest: "sha256:dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd",
    expires_at: "2026-10-30T12:00:00Z"
  }
...
rebind_release_approval_evidence() {
  local dir=$1
  local environment_sha branch_policies_sha approval_history_sha run_identity_sha reviewer_sha authorization_sha
  ...
  approval_history_sha=$(sha256_file "$dir/personal-relay-release-approval-history.json")
  run_identity_sha=$(sha256_file "$dir/personal-relay-release-run-identity.json")
  reviewer_sha=$(sha256_file "$dir/personal-relay-release-reviewer.json")
  jq \
    --arg environment_sha "$environment_sha" \
    --arg branch_policies_sha "$branch_policies_sha" \
    --arg approval_history_sha "$approval_history_sha" \
  ...
  jq \
    ...
    --arg authorization_sha "$authorization_sha" '
      .release_approval.environment_sha256 = $environment_sha
      | .release_approval.branch_policies_sha256 = $branch_policies_sha
      | .release_approval.approval_history_sha256 = $approval_history_sha
      | .release_approval.run_identity_sha256 = $run_identity_sha
      | .release_approval.reviewer_sha256 = $reviewer_sha
      | .release_approval.authorization_sha256 = $authorization_sha
...
bad_release_evidence="$tmp_dir/release-multiple-reviewer-types"
cp -R "$evidence" "$bad_release_evidence"
jq '.protection_rules[0].reviewers += [{type: "Team", reviewer: {login: "release-team", id: 6161, node_id: "MDQ6VGVhbTYxNjE="}}]' \
  "$bad_release_evidence/personal-relay-release-environment.json" > "$tmp_dir/release-multiple-reviewer-types.json"
mv "$tmp_dir/release-multiple-reviewer-types.json" "$bad_release_evidence/personal-relay-release-environment.json"
rebind_release_approval_evidence "$bad_release_evidence"
expect_release_evidence_rejected multiple-reviewer-types "$bad_release_evidence"
...
jq '.release_approval.image_name = "ghcr.io/justinharkelroad/another-personal-relay"' \
...
jq '.release_approval.candidate_tag = "sha-ffffffffffffffffffffffffffffffffffffffff"' \
...
bad_release_evidence="$tmp_dir/release-self-approved"
cp -R "$evidence" "$bad_release_evidence"
jq '.protection_rules[0].reviewers[0].reviewer = {login: "release-actor", id: 1111, node_id: "MDQ6VXNlcjExMTE="}' \
  "$bad_release_evidence/personal-relay-release-environment.json" > "$tmp_dir/release-self-approved-environment.json"
mv "$tmp_dir/release-self-approved-environment.json" "$bad_release_evidence/personal-relay-release-environment.json"
jq '.[0].user = {login: "release-actor", id: 1111, node_id: "MDQ6VXNlcjExMTE="}' \
  "$bad_release_evidence/personal-relay-release-approval-history.json" > "$tmp_dir/release-self-approved-history.json"
mv "$tmp_dir/release-self-approved-history.json" "$bad_release_evidence/personal-relay-release-approval-history.json"
jq '.reviewer = {login: "release-actor", id: 1111, node_id: "MDQ6VXNlcjExMTE="}' \
  "$bad_release_evidence/personal-relay-release-reviewer.json" > "$tmp_dir/release-self-approved-reviewer.json"
mv "$tmp_dir/release-self-approved-reviewer.json" "$bad_release_evidence/personal-relay-release-reviewer.json"
rebind_release_approval_evidence "$bad_release_evidence"
expect_release_evidence_rejected self-approved-release "$bad_release_evidence"
...
bad_release_evidence="$tmp_dir/release-admin-bypass-enabled"
cp -R "$evidence" "$bad_release_evidence"
jq '.can_admins_bypass_exposed = true | .can_admins_bypass = true' \
  "$bad_release_evidence/personal-relay-release-environment.json" > "$tmp_dir/release-admin-bypass-enabled.json"
mv "$tmp_dir/release-admin-bypass-enabled.json" "$bad_release_evidence/personal-relay-release-environment.json"
rebind_release_approval_evidence "$bad_release_evidence"
expect_release_evidence_rejected admin-bypass-enabled "$bad_release_evidence"

bad_release_evidence="$tmp_dir/release-duplicate-approval-history"
cp -R "$evidence" "$bad_release_evidence"
jq '. + [.[0]]' "$bad_release_evidence/personal-relay-release-approval-history.json" \
  > "$tmp_dir/release-duplicate-approval-history.json"
mv "$tmp_dir/release-duplicate-approval-history.json" "$bad_release_evidence/personal-relay-release-approval-history.json"
rebind_release_approval_evidence "$bad_release_evidence"
expect_release_evidence_rejected duplicate-approval-history "$bad_release_evidence"
...
jq --arg sha "$tampered_authorization_sha" '.release_approval.authorization_sha256 = $sha' \
...
for artifact_field in id name digest expires_at; do
  ...
  case "$artifact_field" in
    id) jq '.release_approval.evidence_artifact.id = 0' "$bad_release_evidence/personal-relay-release.json" ;;
    name) jq '.release_approval.evidence_artifact.name = "substituted"' "$bad_release_evidence/personal-relay-release.json" ;;
    digest) jq '.release_approval.evidence_artifact.digest = "sha256:short"' "$bad_release_evidence/personal-relay-release.json" ;;
    expires_at) jq '.release_approval.evidence_artifact.expires_at = "not-a-timestamp"' "$bad_release_evidence/personal-relay-release.json" ;;
...
expect_evidence_rejected() {
  ...
  local approvals=$5
  local main_branch=${6:-"$tmp_dir/main-branch.json"}
  local main_rules=${7:-"$tmp_dir/main-effective-rules.json"}
  local ref_protected=${8:-true}
  local environment_config=${9:-"$tmp_dir/environment.json"}
  local main_rulesets=${10:-"$tmp_dir/main-rulesets.json"}
  local validation_evidence=${11:-"$evidence"}
  local validation_approval=${12:-"$approval"}
  local validation_approval_sha=${13:-"$approval_sha"}
  local validation_approval_secret_proof=${14:-"$tmp_dir/approval-secret-scan-proof.json"}
  if bash "$gate1" validate \
    ...
    --environment-branch-policies "$tmp_dir/branch-policies.json" \
    --environment-approvals "$approvals" \
...
expect_duplicate_approval_rejected() {
  ...
  expect_evidence_rejected "$name" \
    ...
    "$tmp_dir/environment-approvals.json" \
...
expect_evidence_rejected tampered-secret-repodigest "$source_proof/source-test-result.json" "$tmp_dir/tampered-secret-amd64.json" "$tmp_dir/gate1-run-metadata.json" "$tmp_dir/environment-approvals.json"
jq '.run_attempt = 2' "$tmp_dir/gate1-run-metadata.json" > "$tmp_dir/tampered-run-attempt.json"
expect_evidence_rejected tampered-run-attempt "$source_proof/source-test-result.json" "$tmp_dir/secret-amd64.json" "$tmp_dir/tampered-run-attempt.json" "$tmp_dir/environment-approvals.json"
jq '.[0].user.id = 4243' "$tmp_dir/environment-approvals.json" > "$tmp_dir/tampered-reviewer-id.json"
expect_evidence_rejected tampered-reviewer-id "$source_proof/source-test-result.json" "$tmp_dir/secret-amd64.json" "$tmp_dir/gate1-run-metadata.json" "$tmp_dir/tampered-reviewer-id.json"
jq '.protected = false' "$tmp_dir/main-branch.json" > "$tmp_dir/unprotected-main-branch.json"
expect_evidence_rejected unprotected-main "$source_proof/source-test-result.json" "$tmp_dir/secret-amd64.json" "$tmp_dir/gate1-run-metadata.json" "$tmp_dir/environment-approvals.json" "$tmp_dir/unprotected-main-branch.json"
jq 'map(select(.type != "deletion"))' "$tmp_dir/main-effective-rules.json" > "$tmp_dir/weak-main-rules.json"
expect_evidence_rejected weak-main-rules "$source_proof/source-test-result.json" "$tmp_dir/secret-amd64.json" "$tmp_dir/gate1-run-metadata.json" "$tmp_dir/environment-approvals.json" "$tmp_dir/main-branch.json" "$tmp_dir/weak-main-rules.json"
expect_evidence_rejected false-ref-protection "$source_proof/source-test-result.json" "$tmp_dir/secret-amd64.json" "$tmp_dir/gate1-run-metadata.json" "$tmp_dir/environment-approvals.json" "$tmp_dir/main-branch.json" "$tmp_dir/main-effective-rules.json" false
...
jq '.release_approval.evidence_artifact.expires_at = "2026-08-03T12:00:30Z"' \
...
expect_evidence_rejected expired-release-authorization-artifact "$source_proof/source-test-result.json" "$tmp_dir/secret-amd64.json" "$tmp_dir/gate1-run-metadata.json" "$tmp_dir/environment-approvals.json" "$tmp_dir/main-branch.json" "$tmp_dir/main-effective-rules.json" true "$tmp_dir/environment.json" "$tmp_dir/main-rulesets.json" "$expired_authorization_evidence" "$tmp_dir/release-authorization-expired-approval.json" "$expired_approval_sha" "$tmp_dir/release-authorization-expired-secret-proof.json"
jq '.protection_rules[0].reviewers[0].reviewer.id = 9999' "$tmp_dir/environment.json" > "$tmp_dir/mismatched-configured-reviewer.json"
expect_evidence_rejected configured-reviewer-mismatch "$source_proof/source-test-result.json" "$tmp_dir/secret-amd64.json" "$tmp_dir/gate1-run-metadata.json" "$tmp_dir/environment-approvals.json" "$tmp_dir/main-branch.json" "$tmp_dir/main-effective-rules.json" true "$tmp_dir/mismatched-configured-reviewer.json"
jq '.can_admins_bypass_exposed = true | .can_admins_bypass = true' \
  "$tmp_dir/environment.json" > "$tmp_dir/gate1-admin-bypass-enabled.json"
expect_evidence_rejected gate1-admin-bypass-enabled "$source_proof/source-test-result.json" "$tmp_dir/secret-amd64.json" "$tmp_dir/gate1-run-metadata.json" "$tmp_dir/environment-approvals.json" "$tmp_dir/main-branch.json" "$tmp_dir/main-effective-rules.json" true "$tmp_dir/gate1-admin-bypass-enabled.json"
jq '.[0].bypass_actors = [{actor_id: 1, actor_type: "RepositoryRole", bypass_mode: "always"}]' \
  "$tmp_dir/main-rulesets.json" > "$tmp_dir/main-rulesets-bypass.json"
expect_evidence_rejected main-ruleset-bypass "$source_proof/source-test-result.json" "$tmp_dir/secret-amd64.json" "$tmp_dir/gate1-run-metadata.json" "$tmp_dir/environment-approvals.json" "$tmp_dir/main-branch.json" "$tmp_dir/main-effective-rules.json" true "$tmp_dir/environment.json" "$tmp_dir/main-rulesets-bypass.json"
